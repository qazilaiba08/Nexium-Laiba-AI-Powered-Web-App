        ...
        function showAppView(user) {
            ...
            authSection.innerHTML = `
                <div class="flex items-center space-x-4">
                    <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQoFZ5G0prHmBsDr8jkuvQ5cwMBX4IXf3g9Fg&s" alt="User profile initial" class="rounded-full w-10 h-10">
                    <button id="sign-out-btn" class="text-gray-700 hover:text-orange-600 border-b border-grey hover:border-orange-600 transition-colors">
                        Sign Out (${user.email})
                    </button>
                </div>
            `;
        ...
        async function signOut() {
            ...
            if (!error) {
                ...
                authSection.innerHTML = `
                    <button id="sign-in-btn" class="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg font-medium">
                        Sign In
                    </button>
                `;
        ...
        async function generateRecipe() {
            ...
            setTimeout(() => {
                // This would be replaced with actual n8n API call
                const mockRecipe = {
        ...
        function displayRecipe(recipe) {
            recipeResult.innerHTML = `
                <div class="w-full">
                    <h3 class="text-2xl font-bold mb-4 text-gray-800">${recipe.title}</h3>
                    <div class="flex flex-wrap gap-8">
                        <div class="flex-1 min-w-[250px]">
                            <h4 class="text-lg font-semibold mb-2 text-gray-700">Ingredients</h4>
                            <ul class="list-disc pl-5 space-y-1 text-gray-600">
                                ${recipe.ingredients.map(ing => `<li>${ing}</li>`).join('')}
                            </ul>
                        </div>
                        <div class="flex-1 min-w-[250px]">
                            <h4 class="text-lg font-semibold mb-2 text-gray-700">Instructions</h4>
                            <ol class="list-decimal pl-5 space-y-2 text-gray-600">
                                ${recipe.instructions.map((step, i) => `<li>${step}</li>`).join('')}
                            </ol>
                            <div class="mt-4 p-3 bg-orange-50 rounded-lg">
                                <p class="text-orange-700 font-medium">Cook Time: ${recipe.cookTime}</p>
                            </div>
                            <button id="save-recipe-btn" class="mt-4 bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-medium">
                                Save Recipe
                            </button>
                        </div>
                    </div>
        ...
        // Save recipe to MongoDB (via Supabase)
        async function saveRecipe(recipe) {
            const { data: { user } } = await supabase.auth.getUser();
            if (!user) return;

            // This would be replaced with actual Supabase/MongoDB integration
            console.log('Recipe saved:', recipe);
            alert('Recipe saved to your collection!');
            
            // Show saved recipes section if hidden
            ...
            recipeCard.innerHTML = `
                ...
                <p class="text-sm text-gray-600 mb-2">${recipe.cookTime} • ${recipe.ingredients.length} ingredients</p>
            `;
